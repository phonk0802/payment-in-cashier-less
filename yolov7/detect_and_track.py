...
opt = {'weights': 'test_886.pt', 'download': None, 'no_download': None, 'img_size': 640, 'conf_thres': 0.4,
       'iou_thres': 0.45,
       'device': 'cpu', 'view_img': None, 'save_txt': True, 'save_conf': True, 'nosave': None, 'classes': None,
       'agnostic_nms': True,
       'augment': None, 'update': None, 'project': 'runs/detect', 'name': 'track', 'exist_ok': True, 'no_trace': True,
       'colored_trk': True,
       'save_bbox_dim': None, 'save_with_object_id': True}
...
def detect_id(source):
    source, weights, view_img, save_txt, imgsz, trace, colored_trk, save_bbox_dim, save_with_object_id = (
        source, opt['weights'], opt['view_img'], opt['save_txt'], opt['img_size'], not opt['no_trace'], opt['colored_trk'],
        opt['save_bbox_dim'], opt['save_with_object_id'])
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if not os.path.exists(str(save_dir / 'labels' / p.stem)):
                os.mkdir(str(save_dir / 'labels' / p.stem))
            txt_path = str(save_dir / 'labels' / p.stem / p.stem) + ('' if dataset.mode == 'image' else f'_{frame}')  # img.txt
